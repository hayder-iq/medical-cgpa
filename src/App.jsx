...
const STORAGE_KEY = "medical-cgpa-v17";
...
export default function MedIQPro() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ grades, dark }));
    } catch (_) {}
  }, [grades, dark]);
  ...
  const metrics = useMemo(() => {
    ...
    const enteredSubjectsCount = Object.values(grades).filter((v) => v !== "" && v != null).length;
  ...
  const downloadPDF = () => {
    ...
    metrics.perStage.forEach((st) => {
      ...
      doc.text(`${st.full}: ${st.stageDegree.toFixed(2)} / 5`, 20, y);
      y += 10;
    });
